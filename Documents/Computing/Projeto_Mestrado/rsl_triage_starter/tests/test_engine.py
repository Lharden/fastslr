...
import fastslr.engine as engine_module
from fastslr.config import get_domain_blocks, load_global_params, parse_terms_csv
from fastslr.engine import collect_statistics, process_articles
from fastslr.io import compute_config_hash
from fastslr.normalization import NormalizationEngine
from fastslr.patterns import precompile_patterns
...
class TestProcessArticles:
    def test_pipeline_produces_results(
        ...
        result_df, stats = process_articles(
            sample_articles, config, show_progress=False
        )
    ...
    def test_systematic_review_rejected(
        ...
        result_df, _ = process_articles(sample_articles, config, show_progress=False)
    ...
    def test_error_policy_flag_keeps_article(
        ...
        result_df, stats = process_articles(df, cfg, show_progress=False)
    ...
    def test_error_policy_fail_raises(self, sample_config, mini_terms_csv, monkeypatch):
        ...
        with pytest.raises(RuntimeError):
            process_articles(df, cfg, show_progress=False)
